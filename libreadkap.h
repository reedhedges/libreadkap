...
namespace libreadkap {
    ...
    struct KAPData {
        uint16_t width = 0;   ///< number of image rows. from RA field. 
        uint16_t height = 0;  ///< number of image columns. from RA field. 
        double rx = 0.0;        ///< Pixel size (From DX field)
        double ry = 0.0;        ///< Pixel size (From DY field)
        int bits_in = 0;      ///< Pixel depth (bits) of KAP data. From IFM field. TODO is this whole pixel or pixel color component. (E.g. does 8 mean 8 bits per pixel, or 24 bit 888=RGB ?)
        int bits_out = 0;     // todo move out of this into function?
  ...
  /*   // If 0, a limit is ignored.
    ...
    /// Function to read @a nmemb items of data, each @a size bytes long from some file, storing them at @a buf
    ...
    /// Seek function to change internal read pointer position in a file (either offset relative to start, or backwards from end, according to sign of @a pos)
    ...
    */
    ...
    /// Read KAP file via provided functions. Use this to provide an alternate fread()-like and fseek()-like functions to read KAP file data
    /// @param startfn Callback function in which you can create/allocate/prepare your output image data, and receive metadata from the KAP file header.
    /// @param linefn Provide this function, which should return pointers to byte rows in your image data structure.
    ...
    /// @param startfn Callback function in which you can create/allocate/prepare your output image data, and receive metadata from the KAP file header.
    /// @param linefn Provide this function, which should return pointers to byte rows in your image data structure.
    inline Status readkap(std::istream& stream, start_function startfn, get_line_function linefn)
    {
    ...
    /// Read KAP file from std::istream into given buffer.
    /// @param startfn Callback function in which you can create/allocate/prepare your output image data, and receive metadata from the KAP file header.
    inline Status readkap(std::istream& stream, start_function startfn, uint8_t* buffer)
    {
    ...
    /// @param startfn Callback function in which you can create/allocate/prepare your output image data, and receive metadata from the KAP file header.
    /// @param linefn Provide this function, which should return pointers to byte rows in your image data structure.
    inline Status readkap(const std::filesystem::path& filename, start_function startfn, get_line_function linefn)
    {
    ...
    /// Read KAP file with given name on filesystem into given buffer.
    /// @param startfn Callback function in which you can create/allocate/prepare your output image data, and receive metadata from the KAP file header.
    inline Status readkap(const std::filesystem::path& filename, start_function startfn, uint8_t* buffer)
    {
        auto linefn = [buffer](size_t row, const KAPData& kapdata) -> uint8_t* {
            return buffer + (row * kapdata.width);
        };
